  ...
  return (
    <div className="h-full flex flex-col bg-white border-l border-[var(--border)]">
      ...
      <div className="px-4 py-2 border-b border-[var(--border)]">
        <p className="font-mono text-xs text-[var(--text-tertiary)] truncate">{data.fullPath}</p>
      </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              {data.imports.map(imp => (
                <span key={imp} className="text-[11px] font-mono px-2 py-0.5 bg-[var(--bg-secondary)] rounded text-[var(--text-secondary)]">
                  {imp.split('/').pop()}
                </span>
              ))}
            </div>
